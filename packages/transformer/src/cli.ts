...
import type { RawCapture } from '@url-to-figma/capture';
import type { IRNode } from '@url-to-figma/shared';

const [, , inputPath = '../capture/raw-capture.json', outputPath = 'ir.json'] = process.argv;

const raw = JSON.parse(readFileSync(inputPath, 'utf8')) as RawCapture;
const doc = transform(raw);

writeFileSync(outputPath, JSON.stringify(doc, null, 2));

function countTypes(nodes: IRNode[]): Record<string, number> {
