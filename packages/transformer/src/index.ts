...
import type { RawCapture, RawElement, RawAsset, StyleName } from '@url-to-figma/capture';
...
function px(val: string | undefined): number | undefined {
...
function hasVisibleBg(s: Record<StyleName, string>): boolean {
  return !isTransparent(s.backgroundColor);
}

function hasVisibleBorder(s: Record<StyleName, string>): boolean {
...
function hasVisibleRadius(s: Record<StyleName, string>): boolean {
  return [
...
function hasVisualProps(s: Record<StyleName, string>): boolean {
  return hasVisibleBg(s) || hasVisibleBorder(s) || hasVisibleRadius(s);
}

function borderStroke(s: Record<StyleName, string>): { stroke?: string; strokeWidth?: number } {
  const widths = s.borderWidth.trim().split(/\s+/).map(parseFloat);
  const styles = s.borderStyle.trim().split(/\s+/);
...
function mapAlign(val: string): IRTextNode['textAlign'] {
...
function cornerRadii(
  s: Record<StyleName, string>,
): { borderRadius?: number; cornerRadii?: [number, number, number, number] } {
...
function parseBoxShadow(css: string): IRDropShadow[] {
  if (!css || css === 'none') return [];

  const parts = css.split(/,(?![^(]*\))/);
  const result: IRDropShadow[] = [];

  for (const part of parts) {
    const s = part.trim();
    if (!s || s === 'none' || s.includes('inset')) continue;
...
function directionToAngle(dir: string): number {
  ...
  if (d === 'to top') return 0;
  if (d === 'to top right' || d === 'to right top') return 45;
  if (d === 'to right') return 90;
  if (d === 'to bottom right' || d === 'to right bottom') return 135;
  if (d === 'to bottom') return 180;
  if (d === 'to bottom left' || d === 'to left bottom') return 225;
  if (d === 'to left') return 270;
  if (d === 'to top left' || d === 'to left top') return 315;
  return 180;
}

function parseGradientStops(parts: string[]): IRGradientStop[] {
  const raw: Array<{ color: string; position: number }> = parts.map((p) => {
    p = p.trim();
  ...
  // Anchor endpoints
  if (raw.length > 0 && raw[0].position < 0) raw[0].position = 0;
  if (raw.length > 1 && raw[raw.length - 1].position < 0) raw[raw.length - 1].position = 1;

  // Interpolate remaining undefined positions
  for (let i = 1; i < raw.length - 1; i++) {
    if (raw[i].position >= 0) continue;
    let prevI = i - 1;
    while (prevI >= 0 && raw[prevI].position < 0) prevI--;
    let nextI = i + 1;
    while (nextI < raw.length && raw[nextI].position < 0) nextI++;
  ...
  return raw.filter((r) => r.position >= 0) as IRGradientStop[];
}

function parseGradient(css: string): IRLinearGradient | IRRadialGradient | null {
...
function parseBgImage(
  css: string,
  assets: RawAsset[],
): { fill?: IRFill; backgroundSrc?: string } {
  if (!css || css === 'none') return {};
  ...
  for (const layer of layers) {
    ...
    if (urlM) {
      ...
      const asset = assets.find(
        (a) => a.url === imgUrl || a.url.endsWith(imgUrl) || imgUrl.endsWith(a.localPath.split('/').pop()!),
      );
...
function textNode(el: RawElement, id: string): IRTextNode {
  const s = el.styles;
  const effects = parseBoxShadow(s.boxShadow);

  const decLine = s.textDecorationLine ?? '';
  const textDecoration: IRTextNode['textDecoration'] =
    decLine.includes('underline') ? 'underline'
    : decLine.includes('line-through') ? 'line-through'
    : 'none';

  const fontStyleRaw = s.fontStyle ?? '';
  const fontStyle: IRTextNode['fontStyle'] =
    fontStyleRaw.includes('italic') || fontStyleRaw.includes('oblique') ? 'italic' : 'normal';
...
function rectNode(el: RawElement, id: string, assets: RawAsset[]): IRRectNode {
  const s = el.styles;
  ...
  let fill: IRFill | undefined;
  let backgroundSrc: string | undefined;

  if (s.backgroundImage && s.backgroundImage !== 'none') {
    const bg = parseBgImage(s.backgroundImage, assets);
    if (bg.fill) fill = bg.fill;
    if (bg.backgroundSrc) backgroundSrc = bg.backgroundSrc;
  }
...
function imageNode(el: RawElement, id: string, assets: RawAsset[]): IRImageNode {
  const rawSrc = el.src ?? '';
  ...
  return {
    ...
    height: el.rect.height,
    src,
    objectFit: (el.styles.objectFit as IRImageNode['objectFit']) || 'cover',
  };
}
...
function classify(el: RawElement, assets: RawAsset[]): IRNode | null {
  const { tag, text } = el;
  const id = nextId();

  if (tag === 'img') return imageNode(el, id, assets);
  ...
  if (hasVisualProps(el.styles) || hasBgImage) return rectNode(el, id, assets);
...
export function transform(raw: RawCapture): IRDocument {
  _id = 0;

  const { viewport, elements, url, assets, pageBackground } = raw;
  ...
  const pageHeight = elements.reduce(
    (max, el) => Math.max(max, el.rect.y + el.rect.height),
    viewport.height,
  );

  // Screenshot is the bottom (first) layer — gives pixel-perfect visual fidelity.
  // DOM elements (text, rects) are layered on top so they stay editable.
  const allChildren: IRNode[] = [];
  const screenshotAsset = assets.find((a) => a.url === '__screenshot__');
  if (screenshotAsset) {
    const filename = screenshotAsset.localPath.split('/').pop()!;
    allChildren.push({
      id: 'screenshot',
      type: 'image',
      x: 0,
      ...
      height: pageHeight,
      src: `http://localhost:3001/assets/${filename}`,
      objectFit: 'fill',
    });
  }
