...
export type StyleName =
  ...
  // Per-corner radius (more accurate than the shorthand)
...
export type RawElement = {
  tag: string;
  text?: string;
  src?: string;  // absolute URL for <img> (currentSrc), data: URIs supported
...
export async function capture(options: CaptureOptions): Promise<RawCapture> {
  ...
  try {
    ...
    const rawElements = await page.evaluate(
      ({ styleNames, skipTags }: { styleNames: string[]; skipTags: string[] }) => {
    ...
    const result: RawCapture = {
      url,
      viewport: { width: viewportWidth, height: viewportHeight },
      elements: rawElements as RawElement[],
      assets,
      pageBackground,
    };
    ...
    console.log(
      `Captured ${result.elements.length} elements, ${result.assets.length} assets → ${outputPath}`,
    );
    return result;
  } finally {
    await browser.close();
